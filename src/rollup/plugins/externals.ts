...
export function externals (opts: NodeExternalsOptions): Plugin {
  ...
  return {
    ...
    async resolveId (originalId, importer, options) {
      // Skip internals
      if (!originalId || originalId.startsWith("\u0000") || originalId.includes("?") || originalId.startsWith("#")) {
        return null;
      }
      ...
      if (opts.inline.some(i => (id.startsWith(i) || idWithoutNodeModules.startsWith(i)))) {
        return null;
      }

      // Check for explicit externals
      if (opts.external.some(i => (id.startsWith(i) || idWithoutNodeModules.startsWith(i)))) {
    ...
    async buildEnd () {
      ...
      // Trace files
      let tracedFiles = await nodeFileTrace([...trackedExternals], opts.traceOptions)
