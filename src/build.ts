...
export async function writeTypes(nitro: Nitro) {
  const routeTypes: Record<
    string,
    Partial<Record<RouterMethod | "default" | "signatures", string[]>>
  > = {};
  ...
  for (const mw of middleware) {
    ...
    routeTypes[mw.route].signatures ||= [];
    routeTypes[mw.route][method].push(
      `Simplify<Serialize<Awaited<ReturnType<${eventHandlerType}>>>>`
    );
    routeTypes[mw.route].signatures.push(
      `{
        options:
          ${isMethodOptional ? `(${eventHandlerType} extends EventHandler<infer Input> ? undefined extends Input['query'] & Input['body'] ? undefined | BaseFetchOptions : BaseFetchOptions : never)` : 'BaseFetchOptions'} & { method${
            isMethodOptional ? "?" : ""
          }: ${methodType} } & (${eventHandlerType} extends EventHandler<infer Input> ? Input : EventHandlerRequest),
        response: ${eventHandlerType} extends EventHandler<any, infer Output> ? Simplify<Serialize<Awaited<Output>>> : unknown,
      }`
    );
  }
  ...
  const routes = [
    "// Generated by nitro",
    "import type { InternalFetchSignatures, Serialize, Simplify } from 'nitropack'",
    ...
    ...Object.entries(routeTypes).map(([path, methods]) =>
      [
        `    '${path}': {`,
        ...Object.entries(methods).filter(([method]) => method !== 'signatures').map(
    ...
    "  }",
    "  interface InternalFetchSignatures {",
    ...Object.entries(routeTypes).map(([path, methods]) =>
      [
        `    '${path}': ${methods.signatures?.join(" | ")}`,
        "    ",
      ].join("\n")
    ),
    "  }",
