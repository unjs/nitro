...
You can specify for a route that will only be included in development or production builds by adding a `.dev` or `.prod` suffix to the file name, for example: `routes/test.get.dev.ts` or `routes/test.get.prod.ts`.
