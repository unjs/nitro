...
The entry must export a `fetch` handler, either as a `default` export object (`export default { fetch }`) or as a named `fetch` export. When both are present, the `default` export wins.
