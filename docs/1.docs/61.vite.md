...
The entry (like any other service entry) must export a `fetch` handler, as a `default` export object (`export default { fetch }`), a default function (`export default (req: Request) => Response`), or a named `fetch` export. When both are present, the `default` export wins.
