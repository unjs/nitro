...
The server entry is imported eagerly, so its top-level code runs **once when the server starts**, not on the first request. This makes the module scope a good place for one-time setup such as creating a framework app, reading configuration, or opening connections:
...
- Put one-time initialization in the module scope of the server entry, or in [runtime plugins](/docs/plugins) when it needs hooks or `useNitroApp()`
