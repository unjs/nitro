...
- During development (`nitro dev`), the dev server address is controlled by the CLI (`--port`, `--host`) and currently only `middleware` and `plugins` are applied to the worker server. The Vite dev server applies none of them.
