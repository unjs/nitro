...
- During development (`nitro dev`), only `middleware` and `plugins` are applied; the dev server address is controlled by the CLI (`--port`, `--host`). The Vite dev server applies none of them.
