...
Prerendered options. Any route specified will be fetched during the build and copied to the `.output/public` directory as a static asset. If `stdout` is set to `true` the prerendered HTML will be printed to the console instead of writing to the file system. To only output the HTML and not logs, set `logLevel` to `0`.
